public class Burger {
    ...
    // дайот возможность создать 3-й конструктор где бургер с двойным мясом и выполнить условие задачи
    String meat;
    String cheese;
    String greens;
    String mayonnaise;
...
Пробовал другими путями выполнить:
    ...
    - пробовал через boolean doubleMeat (через static метод и без него внутри конструктора) который в зависимомти
      от того что пришло менял на false или true (используя условную инструкцию if (if else)) и далее сложная логика в конструкторе
    - пробовал через тернарную операцию определять что пришло meat или doubleMea (через static метод и без него внутри конструктора), типа так:
                        boolean doubleMeat = false;
                        String meat = meat ? doubleMeat = false : doubleMeat = true;
